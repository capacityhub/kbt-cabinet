<template>
  <Row type="flex" justify="center" align="middle" class="regist" @keydown.enter.native="submitRegist">
    <Col :xs="{span:22}" class="regis-col">
      <Row class="header">
      ...
      <Row class="regist-form">
        <Form ref="registForm" :model="form" :rules="rules" class="form">
          ...
          <FormItem prop="verifyCode" :error="errorCode">
            <Row type="flex" justify="space-between" class="code-row-bg">
              <Col :xs="{span:16}"><Input v-model="form.verifyCode" size="large" clearable placeholder="请输入短信验证码(随意输入即可)" :maxlength="10" class="input-verify" /></Col>
              <Col :xs="{span:7}"><Button size="large" @click="sendVerify" v-if="!sended" class="send-verify">获取验证码</Button></Col>
              <Col :xs="{span:7}"><Button size="large" disabled v-if="sended" class="count-verify">{{countButton}}</Button></Col>
            </Row>
          </FormItem>
          <FormItem prop="verify">
            <Row type="flex" justify="space-between" class="code-row-bg" style="align-items: center;overflow: hidden;">
              <Col :xs="{span:16}"><Input v-model="form.verify" size="large" clearable placeholder="请输入验证码" :maxlength="10" class="input-verify" /></Col>
              <Col :xs="{span:7}">
        ...
        <Row class="code-row-bg">
          ...
          <router-link to="/login"><div class="to-login">已有账号登录</div></router-link>
        </Row>
      </Row>
      ...
      <!--<Row class="foot">-->
        ...
        <!--<Row type="flex" justify="center" class="code-row-bg copyright">-->
          <!--Copyright © 2019 - Present <a href="#" target="_blank" style="margin:0 5px;">{{ $t('power') }}</a> 版权所有-->
        <!--</Row>-->
      <!--</Row>-->
    </Col>
  </Row>
