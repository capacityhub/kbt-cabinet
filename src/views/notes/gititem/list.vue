  ...
  <div class="search">
    ...
    >
      <Form ref="dataFrom" :model="dataFrom" :label-width="70" :rules="dataFromValidate">
        ...
        <FormItem label="git地址" prop="url">
          <Input v-model="dataFrom.url" autocomplete="off"/>
        </FormItem>
        <FormItem label="用户名" prop="user">
          <Input v-model="dataFrom.user" autocomplete="off"/>
        </FormItem>
        <FormItem label="密码" prop="pass">
          <Input v-model="dataFrom.pass" autocomplete="off"/>
        </FormItem>
      </Form>
      <div slot="footer">
...
export default {
  ...
  methods: {
    ...
    getGitItemList() {
      ...
      getGitItemListData(this.searchForm).then(res => {
        this.loading = false;
        if (res.success === true) {
      ...
      this.$refs.dataFrom.validate(valid => {
        if (valid) {
          if (this.modalType === 0) {
            ...
            addGitItem(this.dataFrom).then(res => {
            ...
            // 编辑
            this.submitLoading = true;
            editGitItem(this.dataFrom).then(res => {
              this.submitLoading = false;
              if (res.success === true) {
                this.$Message.success("操作成功");
                this.getGitItemList();
